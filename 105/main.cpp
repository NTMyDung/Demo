...
int main()
{
    ...
    for(int i=n; i>=1; i--){   //duyet nguoc mang de tim so hoan thien cuoi cung
        if(a[i]>0){
            ...
            if(tong==a[i]) {            // xet dieu kien cua so hoan thien
                    cout<<a[i];          // xuat ra so hoan thien cuoi cung trong mang
                    dem++;              // bien dem de biet co ton tai so hoan thien trong mang
                    break;              // ket thuc vong lap
            }
        }
    }
    if(dem==0) cout<<-1;        //neu mang khong co so hoan thien thi cout -1
    return 0;
}
